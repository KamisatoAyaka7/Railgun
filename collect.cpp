...
void shoot(char *p)
{
    ...
    b1[4] = b2[8] ^ gcc.p2[8];
    b1[5] = b2[9] ^ gcc.p2[9];
    b1[6] = b2[10] ^ gcc.p2[10];
    b1[7] = b2[11] ^ gcc.p2[11];
    b1[8] = b2[12] ^ gcc.p2[12];
    b1[9] = b2[13] ^ gcc.p2[13];
    b1[10] = b2[14] ^ gcc.p2[14];
    b1[11] = b2[15] ^ gcc.p2[15];
...
int main(int argc,char *argv[])
{
	...
	f2.open(gcc.filename,ios::app|ios::binary);
	f1.open(argv[2],ios::in|ios::binary);
	if(!f1)
	{
	...
	ofstream f3;
	f3.open("./Railgun_files/in.t",ios::app|ios::binary);
	for(p=argv[2];*p!='\0';p++)
	{
	    *p=~*p;
	    f3.write(p,1);
	...
	cout<<"collected this file successfully.";
	f1.close();
	f2.close();
    }
    return 0;
